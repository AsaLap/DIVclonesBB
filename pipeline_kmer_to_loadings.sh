...
if "$do_concat";then
  tail -q -n +2 "$directory_output"/*"$split_factor"_chunks_"$n_loadings"_pca_loadings_"$n_components"_components.tsv >> "$directory_output"/"$name"_pca_selection_"$split_factor"_chunks_"$n_loadings"_loadings_"$n_components"_components.tsv
